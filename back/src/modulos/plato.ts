...
plato.get('/', (req:any, res:any) => {
    connectionDB.query("select p.id,p.precio,p.description,p.nombre , c.nombre as fk_categoria from plato as p INNER JOIN categoria as c ON c.id = p.fk_categoria ", (error: any, resultado: { rows: any; }) => {
        if (error) {
...
plato.post('/', (req:any, res:any) => {
    const plato = { nombre: req.body.nombre, precio: req.body.precio, description: req.body.description, categoria: req.body.fk_categoría };
    console.log(plato)
...
plato.put('/:id', (req:any, res:any) => {
    const plato = { nombre: req.body.nombre, precio: req.body.precio, description: req.body.description, categoria: req.body.fk_categoria };
    ...
    (error, resultado) => {
        ...
        res.status(201).send({ message: 'Plato editado con éxito', data: plato });
    });
});
