...
orden.delete('/:id', (req:any, res:any) => {

    connectionDB.query('DELETE FROM orden WHERE id =$1',[req.params.id], (error:any, result:any) => {
        if (error) {
            console.error('Error al obtener las órdenes:', error);
            return res.status(400).send({ message: 'Error al obtener las órdenes', error });
        } 
        res.status(200).json({message:"eliminado correctamentre"});
    });
});
